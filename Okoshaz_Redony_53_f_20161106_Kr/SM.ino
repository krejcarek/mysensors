...
void ShutterTimerSetUp(uint8_t shutter, uint32_t time) {
  int offset = EEPROM_OFFSET_SKETCH + EEPROM_POSITION_OFFSET * shutter;
  writeEeprom2Byte(offset + EEPROM_TRAVELUP_OFFSET, time);
}

void ShutterTimerSetDown(uint8_t shutter, uint32_t time) {
  int offset = EEPROM_OFFSET_SKETCH + EEPROM_POSITION_OFFSET * shutter;
  writeEeprom2Byte(offset + EEPROM_TRAVELDOWN_OFFSET, time);
}
