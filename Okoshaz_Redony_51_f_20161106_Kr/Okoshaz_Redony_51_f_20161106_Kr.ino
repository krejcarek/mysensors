...
void receive(const MyMessage &message) {
  ...
  if (message.isAck()) {}
    ...
    if (message.type == V_PERCENTAGE) {
       if (message.getByte() >= 100) setPosition(message.sensor,105);
       else{
       if (message.getByte() <= 0) setPosition(message.sensor,-5); else setPosition(message.sensor,message.getByte());
       }
       #ifdef MY_DEBUG_SKETCH 
